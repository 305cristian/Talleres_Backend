...
talleresCtrl.editTaller = async (req, resp) => {
    var {title, description,tiempo,intentos, area_id, image, video} = req.body;
...
//        }else if(ext==='.mp4' || ext==='.vlc' || ext==='.avi'){
    ...
    const  newTaller = ({title: title, description: description,tiempo:tiempo,intentos:intentos, area_id: area_id, image: image, video: video});
...
talleresCtrl.createTaller = async (req, resp) => {
    ...
    const{title, description,tiempo,intentos, area_id, video} = req.body;
    console.log(req.body);
    console.log(req.files['image'][0]);
    const taller = new Taller({title: title, description: description,tiempo:tiempo,intentos:intentos, area_id: area_id, video: video});
    if (req.files) {
        const{filename} = req.files['image'][0];
